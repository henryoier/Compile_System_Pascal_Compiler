#ifndef _pascompile_semanticANLS_H_
#define _pascompile_semanticANLS_H_
...
extern pascompile::program *savedRoot;
extern bool error;

pascompile::program *parse();
void lower(char str[]);
void reportNameCollision(pascompile::identifier *id, int scope);
void reportNameMissing(pascompile::identifier *id, int scope);
void reportTypeMismatch(pascompile::identifier *id, int scope);
...
void reportTypeMismatch(int lineno, pascompile::identifier *id, Type type);
std::string typeString(pascompile::identifier *id);
std::string typeString(expression *exp);
std::string typeString(expr *exp);
std::string tString(Type type, SimpleType stype);
bool typeEqual(pascompile::identifier *id, expression *exp);
bool typeEqual(pascompile::identifier *id1, pascompile::identifier *id2);
...
struct label_part {
	...
	std::string codeGenerate(){return "";}
};
...
struct simple_type_decl {
	...
	union {
		sys_type *choice1;
		pascompile::identifier *choice2;
		name_list *choice3;
		range *choice4;
		idrange *choice5;
	} child1;
	simple_type_decl(sys_type *c1):choice1(1) { child1.choice1 = c1; }
	simple_type_decl(pascompile::identifier *c2):choice1(2) { child1.choice2 = c2; }
	...
	bool semanticCheck(identifier *id, int scope, int type);
	bool semanticCheck(name_list *ids, int scope, int type);
	bool semanticCheck(identifier *id, int scope, int type, int *fatherchoice);
	identifier* getArrayDecl(int scope);
	std::string codeGenerate(){return "";}
};
...
struct sys_proc {
	...
	std::string codeGenerate(SimpleType t);
};
