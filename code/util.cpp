...
void reportNameCollision(pascompile::identifier *id, int scope) {
	pascompile::identifier *temp = gtable.lookUp(id, scope);
	...
	cerr << "error at line "<< id->lineno <<" : "<< temp->name.c_str() <<" was previously defined at line "<< temp->lineno <<"." << endl;
}

void reportNameMissing(pascompile::identifier *id, int scope) {
	assert(gtable.lookUp(id, scope)==NULL);
	cerr << "error at line "<< id->lineno <<" : "<< id->name.c_str() <<" is used but never defined." << endl;
}
...
void reportTypeMismatch(pascompile::identifier *id, int scope) {
	if(id->type==ssimple_type&&id->stype==iidref) {
		...
		cerr << "error at line "<< id->lineno <<" : need "<< id->value.idref->name.c_str() <<" be a type, but was previously defined not." << endl;
	}
}
